...
public class WorldResourceObject implements ResourceObject
{
	...
	protected void init(Vector pos, Rectangle bound,String params) {
	...
	protected WorldResourceObject set_child(int i, WorldResourceHandler WRH, Vector pos, Rectangle bound, String params) {
	...
	protected void disable_child(int i) {
		children[i].WRH.disable_resource_object(children[i]);
	}
	
	protected void disable_children() {
		for(int i = 0; i < children.length; i++) {
			if(children[i] != null) {
				disable_child(i);
			}
